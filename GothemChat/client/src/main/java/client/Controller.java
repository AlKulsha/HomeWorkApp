...
public class Controller implements Initializable {
    ...
    public void clickBtnAuth(ActionEvent actionEvent) {
        ...
        try{
            String msg = String.format("%s %s %s",
                    ServiceMessages.AUTH, loginField.getText().trim(), passwordField.getText().trim());
            out.writeUTF(msg);
            passwordField.clear();
        } catch (IOException e) {
            e.printStackTrace();
    ...
    private void setTitle(String nickname){
        ...
        if(nickname.equals(" ")){
            ...
            title = String.format("GothemChat - %s", nickname);
        }
        Platform.runLater(() -> {
    ...
    private void createRegWindow(){
        try {
            ...
            regStage = new Stage();
            regStage.setTitle("GothemChat registration");
            regStage.setScene(new Scene(root, 500, 475));

            regStage.initModality(Modality.APPLICATION_MODAL);  // позволяет держать окно на переднем плане
            regStage.initStyle(StageStyle.UTILITY);   // маленький крестик для закрытия
